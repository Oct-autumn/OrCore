//! fs/src/ex_fat/cluster_chain/up_case_table
//!
//! 大写字符映射表、文件名哈希
...
use crate::ex_fat::model::index_entry::{IndexEntry, IndexEntryType};
use crate::ex_fat::model::unicode_str::UnicodeString;
use alloc::vec::Vec;
use spin::rwlock::RwLock;

const DEFAULT_UPCASE_TBL_COMP: [u16; 2918] = [
...
const BAD_UNICODE_CHARS: [u16; 10] = [
...
pub struct UpCaseTable(pub [u16; 0x10000]);

impl UpCaseTable {
    /// 将Unicode字符串转成大写字符串
    pub fn to_upper(&self, uni_str: &UnicodeString) -> UnicodeString {
        let mut upper_str = UnicodeString::new();
        for uni in uni_str.data.iter() {
            upper_str.push(
                if self.0[*uni as usize] == 0 {
                    // 未找到映射值，直接返回原值
                    *uni
                } else {
                    // 找到映射值，返回映射值
    ...
    fn default() -> Self {
        // 大写字毋表
        let mut up_case_table = [0u16; 0x10000];
        // 大写字母表的内部索引（用于指示写入位置）
        let mut index = 0usize;
        // 流程控制变量：标记下一个值是否为“跳过的数量”
        let mut skip = false;
        ...
        for i in 0..2918 {
            ...
            let u16_data = DEFAULT_UPCASE_TBL_COMP[i];
            if skip {
                // 当前值为跳过的数量，将索引值向后移动
                index += u16_data as usize;
                skip = false;
            } else if u16_data as usize == index {
                // 当前值等于索引值，表示该字符不区分大小写，直接跳过
                index += 1;
            } else if u16_data == 0xFFFF {
                // 当前值为0xFFFF，表示后续一个值为跳过的数量
                skip = true;
            } else {
                // uni != index , uni != 0xFFFF
                // 当前值为一个映射值
                up_case_table[index] = u16_data;
                index += 1;
            }
        }
    ...
    /// 从介质中加载大写字母表
    pub fn load_up_case_table(up_case_index_entry: &IndexEntry, cluster_manager: &Arc<RwLock<ClusterManager>>) -> Option<Self> {
        // 大写字母表的目录项
        let up_case_custom = unsafe { &up_case_index_entry.custom_defined.up_case };
        let sector_per_cluster = cluster_manager.read().sectors_per_cluster;
        let tbl_cluster_id = ClusterId(up_case_custom.start_clu);
        let tbl_sector_count = (up_case_custom.size as usize + config::SECTOR_BYTES - 1) / config::SECTOR_BYTES;
        
        // 校验和
        let mut checksum = UpCaseChecksum::default();
        // 大写字毋表
        let mut up_case_table = [0u16; 0x10000];
        // 大写字母表的内部索引（用于指示写入位置）
        let mut index = 0usize;
        ...
        let mut pos = (tbl_cluster_id, 0);

        while sector_count < tbl_sector_count {            
            let Some(sector) = cluster_manager.read().get_cluster_sector(&pos.0, pos.1) else {
                // 获取扇区缓存失败，终止读取
                break;
            };

            sector.read().read(0, |data: &[u8; config::SECTOR_BYTES]| {
                let mut len = config::SECTOR_BYTES;
                for i in 0..(config::SECTOR_BYTES >> 1) {
                    if index > 0xFFFF {
                        len = i << 1;
                        break;
                    }

                    let u16_data = <u16>::from_le_bytes([data[i * 2], data[i * 2 + 1]]);
                    if skip {
                        // 当前值为跳过的数量，将索引值向后移动
                        index += u16_data as usize;
                        skip = false;
                ...
                checksum.add_bytes(data, len);  // 更新校验和
            });
            
            sector_count += 1;  // 读取扇区数量+1
            pos.1 += 1;  // 移动到下一个扇区
            if pos.1 >= sector_per_cluster {
                pos.1 = 0;
                // 前往下一簇
                pos.0.0 += 1;
            }
        ...
        // 若加载成功，则索引index应位于0xFFFF
        if index >= 0xFFFF {
            // 检查校验和
            if checksum.0 == up_case_custom.checksum {
                // 大写字母表加载成功
                return Some(Self(up_case_table));
            }
        }
        None
    }

    pub fn create(boot_sector: &BootSector, cluster_manager: &Arc<RwLock<ClusterManager>>) -> Self {
        // 定位到根目录，在根目录中搜索大写字母表的索引（类型为82H）
        let mut cluster_id = ClusterId(boot_sector.first_cluster_of_root_directory);
        let sector_per_cluster = 1usize << boot_sector.sectors_per_cluster_shift;

        let mut end_search = false;

        'outer: while !cluster_id.is_invalid() {
            // 遍历簇内的所有扇区
            for sector_offset in 0..sector_per_cluster {
                let Some(sector) = cluster_manager.read().get_cluster_sector(&cluster_id, sector_offset) else {
                    panic!("Failed to get cluster sector");
                };
                if let Some(up_case_table) = sector.read().read(0, |entries: &[IndexEntry; config::SECTOR_BYTES >> 5]| {
                    for entry in entries.iter() {
                        match entry.entry_type {
                            ...
                            IndexEntryType::ExfatUpcase => {
                                // 读取大写字母表                                
                                if let Some(table) = Self::load_up_case_table(entry, cluster_manager) {
                                    // 加载成功，返回大写字母表
                                    return Some(table);
                                } else {
                                    // 加载失败，终止搜索
            ...
            // 前往下一簇
            if let Some(next_cluster_id) = cluster_manager.read().get_next_cluster(&cluster_id) {
...
pub struct UpCaseChecksum(pub u32);

impl UpCaseChecksum {
    pub fn add_bytes(&mut self, sector: &[u8], len: usize) {
        assert!(sector.len() >= len); // 输入的slice大小必须大于等于len
        let mut checksum: u32 = self.0;

        sector.iter().for_each(|byte| {
...
impl FileNameHash {
    pub fn add_uni_string(&mut self, up_case_table: &UpCaseTable, unicode_string: &UnicodeString) {
        let up_case_uni_string = up_case_table.to_upper(unicode_string);
        let bytes = up_case_uni_string.to_le_bytes();

        for i in 0..bytes.len() {
            self.0 = ((self.0 & 1) << 15) + (self.0 >> 1) + bytes[i] as u16;
        }
    }
}